...
class Experiment(abc.ABC):
    ...
    def evaluate(self) -> Dict:
        ...
        max_eval_samples = (
            self.data_args.max_eval_samples if self.data_args.max_eval_samples is not None else num_eval_samples
        )
    ...
    def get_collator(self, tokenizer: transformers.PreTrainedTokenizer) -> transformers.DataCollatorForSeq2Seq:
    ...
    def _load_train_dataset_uncached(
        ...
        if self.model_args.use_frozen_embeddings_as_input:
            print(f"[Precomputing embeddings with batch size: {self.training_args.per_device_train_batch_size}]")
    ...
    def load_train_and_val_datasets(
        ...
        train_dataset_path = os.path.join(DATASET_CACHE_PATH, (md5_hash_kwargs(**train_dataset_kwargs) + ".arrow"))
...
class InversionExperiment(Experiment):
    ...
    def load_trainer(self) -> transformers.Trainer:
        ...
        if self.training_args.mock_embedder:
            ...
            assert model.config.use_frozen_embeddings_as_input, "must use frozen embeddings if mock_embedder=True"
            print(
