...
class InversionFromLogitsModel(InversionModel):
    def __init__(self, config: InversionConfig):
        ...
        self.latent_dim = (
            self.embedder_dim
            if config.embeddings_from_layer_n is not None
            else self.embedder.config.vocab_size
        )

        self.num_zeros_to_add = encoder_hidden_dim - (
            (self.latent_dim + encoder_hidden_dim) % encoder_hidden_dim
        )
        self.num_repeat_tokens = round(
            (self.latent_dim + self.num_zeros_to_add) / encoder_hidden_dim
        )
        ...
        self.unigram = nn.Parameter(
            torch.zeros(
                (1, self.latent_dim + self.num_zeros_to_add),
                dtype=torch.float32,
            ),
            requires_grad=False,
        )
    ...
    def _process_embedder_output(
        ...
        if self.embeddings_from_layer_n is not None:
            assert (
                hasattr(outputs, "hidden_states") and outputs.hidden_states
            ), "output missing hidden states - did you remember to initialize the model with output_hidden_states=True?"
            latent = outputs.hidden_states[self.embeddings_from_layer_n]
        else:
            latent = outputs.logits

        B = len(attention_mask)
        logits = latent[torch.arange(B), attention_mask.sum(1) - 1]
